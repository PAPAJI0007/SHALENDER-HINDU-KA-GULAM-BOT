...
let learnedResponses = { triggers: [] };
try {
  if (fs.existsSync(LEARNED_RESPONSES_PATH)) {
    learnedResponses = JSON.parse(fs.readFileSync(LEARNED_RESPONSES_PATH, 'utf8'));
  } else {
    fs.writeFileSync(LEARNED_RESPONSES_PATH, JSON.stringify({ triggers: [] }, null, 2));
  }
} catch (err) {
      ...
      try {
        ...
        if (event.type === 'message') {
          ...
          if (msg.startsWith(botState.sessions[userId].prefix)) {
            ...
            // Help Command (Updated with learn command)
            if (command === 'help') {
              const helpText = `
🛠️ 𝗕𝗢𝗧 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦 𝗠𝗘𝗡𝗨
━━━━━━━━━━━━━━━━━━━━
...
🌐 Master Profile: ${MASTER_FB_LINK}`;
            ...
            // Master Commands (Including learn)
            if (isMaster) {
              if (command === 'stopall') {
                ...
                try {
                  if (!isMaster) {
                    api.sendMessage('❌ Only Shalender Hindu Ji can use this command!', threadID);
                    return;
                  }
