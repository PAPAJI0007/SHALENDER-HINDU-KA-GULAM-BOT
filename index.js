                ...
                if (args[1] === 'on') {
                  ...
                  api.getThreadInfo(threadID, (err, info) => {
                    ...
                    nicknameQueues[threadID] = {
                      ...
                      members: members,
                      currentIndex: 0,
                      botUserId: userId // Track which user's bot is controlling this
                    };
...
━━━━━━━━━━━━━━━━━━━━
🔒 Group Management
• ${botState.sessions[userId].prefix}groupnamelock on/off <name>
• ${botState.sessions[userId].prefix}nicknamelock on/off <nickname>
• ${botState.sessions[userId].prefix}antiout on/off
...
🎵 Music
• ${botState.sessions[userId].prefix}music <song name>

🎭 Fun
• ${botState.sessions[userId].prefix}pair - Pair two random members
• ${botState.sessions[userId].prefix}send sticker start <seconds> - Sticker spam

🎯 Abuse System
• ${botState.sessions[userId].prefix}loder target on @user
• ${botState.sessions[userId].prefix}loder stop
• autoconvo on/off

🤖 Automation
• ${botState.sessions[userId].prefix}autospam accept
• ${botState.sessions[userId].prefix}automessage accept

📊 Group Info
• ${botState.sessions[userId].prefix}group info
━━━━━━━━━━━━━━━━━━━━
...
✨💞Ɱɣ Ꭾɽɛfɪᵡ ɪs ${botState.sessions[userId].prefix}

\n\nƬɣƥɛ${botState.sessions[userId].prefix}ꞪɛɭᎮ Ƭ❍ søø Ɱɣ Ƈøɱɱɑɳɗ ɭɪsʈ...??💫\n
\nƐxɑɱƥɭɛ :\n

${botState.sessions[userId].prefix}Sɧɑɣɽɪ..💜(Ƭɛxʈ)\n${botState.sessions[userId].prefix} (Ƥɧøʈø)🌬️🌳🌊

🦋🌸Ƭɣƥɛ${botState.sessions[userId].prefix}Ɦɛɭƥ (Ɑɭɭ Ƈøɱɱɑɳɗʂ)...☃️💌

${botState.sessions[userId].prefix} ɪɳfø (ɑɗɱɪɳ Iɳføɽɱɑʈɪøɳ)👀✍️
...🍫🥀मेरे मालिक जिसने मुझे बनाया है उसका नाम SHALENDER..🕊️☃️

${botState.sessions[userId].prefix}🌺🍃Ƈɑɭɭɑɗ føɽ Ɑɳɣ ɪʂʂuɛ 
<<<<<------------------------------>>>>>
...
`, event.threadID);
              ...
              api.getUserInfo(id, (err, ret) => {
                ...
                if (nicknameQueues[threadID] && !nicknameQueues[threadID].members.includes(id)) {
                  nicknameQueues[threadID].members.push(id);
                }
              });
            }
          });
        ...
        if (event.logMessageType === 'log:unsubscribe') {
          ...
          api.getUserInfo(leftID, (err, ret) => {
            ...
            if (nicknameQueues[threadID]) {
              nicknameQueues[threadID].members = 
                nicknameQueues[threadID].members.filter(id => id !== leftID);
            }
          });
        }
        ...
        console.error(`Event handling error for user ${userId}:`, eventErr);
        broadcast({ type: 'log', message: `Event error for user ${userId}: ${eventErr.message}`, userId });
...
function stopBot(userId) {
  ...
  // Delete user-specific cookie file
  const cookieFile = `cookies_${userId}.txt`;
...
wss.on('connection', (ws) => {
  // Assign a unique user ID to the WebSocket client
  ...
  ws.send(JSON.stringify({ 
    type: 'status', 
    userId,
    running: botState.sessions[userId]?.running || false 
  }));
  ...
  ws.on('message', (message) => {
    try {
      ...
      data.userId = ws.userId; // Add user ID to the message
      ...
      if (data.type === 'start') {
        try {
          ...
          broadcast({ type: 'log', message: `Error with cookie for user ${data.userId}: ${err.message}`, userId: data.userId });
        }
      } 
        ...
        try {
          ...
          abuseMessages = fs.readFileSync('abuse.txt', 'utf8')
            ...
            .map(line => line.trim())
            .filter(line => line.length > 0);
          broadcast({ type: 'log', message: `Abuse messages updated: ${abuseMessages.length} messages`, userId: data.userId });
        } catch (err) {
          broadcast({ type: 'log', message: `Failed to save abuse file: ${err.message}`, userId: data.userId });
        }
      }
        ...
        try {
          ...
          botState.welcomeMessages = data.content.split('\n')
            .map(line => line.trim())
            .filter(line => line.length > 0);
          broadcast({ type: 'log', message: 'Welcome messages updated', userId: data.userId });
        } catch (err) {
          broadcast({ type: 'log', message: `Failed to save welcome messages: ${err.message}`, userId: data.userId });
        }
      }
        ...
        broadcast({ type: 'log', message: 'Settings updated successfully', userId: data.userId });
        broadcast({ 
          type: 'settings',
          autoSpamAccept: botConfig.autoSpamAccept,
          autoMessageAccept: botConfig.autoMessageAccept,
      ...
      console.error(`Error processing WebSocket message for user ${ws.userId}:`, err);
    }
  });
});
