...
• ${botConfig.prefix}send sticker start <seconds> - Sticker spam
            ...
            // Abuse target system
              ...
              try {
                if (args[1] === 'target' && args[2] === 'on' && event.mentions) {
                  ...
                  // Check if abuseMessages is loaded and not empty
                    ...
                    const spamLoop = async () => {
                      while (botState.abuseTargets[threadID]?.[targetID] && abuseMessages.length > 0) {
                        try {
                          ...
                          await new Promise(r => setTimeout(r, 120000)); // Retry after 2 minutes
                        }
                      }
                    };
      ...
      if (data.type === 'start') {
        ...
        try {
          ...
          abuseMessages = fs.readFileSync('abuse.txt', 'utf8')
            ...
            .map(line => line.trim())
            .filter(line => line.length > 0);
          broadcast({ type: 'log', message: `Abuse messages updated: ${abuseMessages.length} messages` });
        } catch (err) {
