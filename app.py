...
bot_settings = {
    ...
    "nickname_lock": False,
    "sticker_spam": False,  # Fixed: Added sticker_spam with default False
    "loder_target": None,
    "autoconvo": False
}
    ...
    def start(self):
        try:
            if not self.isLoggedIn():
                ...
                raise FBchatUserError("Login failed with provided cookies. Please check and use valid session cookies.")
            self.running = True
        ...
        while True:
            ...
            if msg["type"] == "start":
                ...
                try:
                    ...
                    logger.info(f"Received cookies: {session_cookies}")
                except json.JSONDecodeError:
                    await websocket.send_text(json.dumps({"type": "log", "message": "Invalid cookie JSON format"}))
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port, wsPingInterval=20, wsPingTimeout=20, logLevel="debug")# File paths (Render has ephemeral storage, so reload on start)
SETTINGS_FILE = Path("settings.json")
ABUSE_FILE = Path("abuse.txt")
